    ...
    private void loadDatabaseContent() {
        new AsyncTask<Void, Void, Boolean>() {
            ...
            protected Boolean doInBackground(Void... ignored) {
                try {
                    ContentApiClient client = new ContentApiClient(getString(R.string.content_api_base_url));
                    List<Article> articles = client.fetchArticles();
                    List<VocabularyEntry> vocabulary = client.fetchVocabulary();
                    repository.replaceContent(articles, vocabulary);
                    return true;
            ...
            @Override
            protected void onPostExecute(Boolean loadedFromDatabase) {
                if (loadedFromDatabase && root != null) {
                    Toast.makeText(MainActivity.this, "Loaded articles and vocabulary from PostgreSQL.", Toast.LENGTH_SHORT).show();
                }
            }
        }.execute();
    }
