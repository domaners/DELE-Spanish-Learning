...
final class LearningRepository {
    ...
    synchronized void replaceContent(List<Article> remoteArticles, List<VocabularyEntry> remoteVocabulary) {
        if (!remoteArticles.isEmpty()) {
            articles = Collections.unmodifiableList(new ArrayList<>(remoteArticles));
        }
        if (!remoteVocabulary.isEmpty()) {
            vocabulary = Collections.unmodifiableList(new ArrayList<>(remoteVocabulary));
        }
    }
